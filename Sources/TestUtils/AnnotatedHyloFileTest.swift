...
extension XCTestCase {
  ...
  private struct ProcessingEffects: Sendable {

    /// Test failures generated by processing.
    let testFailures: [XCTIssueHylo]
  ...
  private func checkAnnotatedHyloFileDiagnostics(
    ...
    if (thrownError == nil) != (expectation == .success) {
      record(XCTIssueHylo(unexpectedOutcomeDiagnostic(thrownError: thrownError, at: f.wholeRange)))
    }
  }
  ...
  private func failuresToReport(
    ...
    testFailures += diagnosticsByExpectation.values.joined().lazy.map {
      XCTIssueHylo(.error("unexpected diagnostic: '\($0.message)'", at: $0.site, notes: $0.notes))
    }
    return testFailures
  }
