...
public func modify<T, U>(_ value: inout T, _ action: @Sendable (inout T) throws -> U) rethrows -> U {
  try action(&value)
}

/// Assigns `value` to the result of applying `transform` on it.
public func update<T>(_ value: inout T, with transform: @Sendable (T) throws -> T) rethrows {
