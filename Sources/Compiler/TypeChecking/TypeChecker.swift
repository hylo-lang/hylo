...
public struct TypeChecker {
  ...
  public func canonicalize(type: AnyType) -> AnyType {
    ...
    case let t as BoundGenericType:
      ...
      let arguments = t.arguments.map({ (a) -> BoundGenericType.Argument in
        switch a {
        case .type(let a): return .type(canonicalize(type: a))
    ...
    case let t as ExistentialType:
      return ^ExistentialType(
        traits: t.traits, constraints: ConstraintSet(t.constraints.map(canonicalize(constraint:))))
    ...
    case let t as UnionType: return ^UnionType(Set(t.elements.map(canonicalize(type:))))
  ...
  mutating func conformedTraits<S: ScopeID>(of type: AnyType, inScope scope: S) -> Set<TraitType>? {
    ...
    case let t as ProductType:
      ...
      guard let traits = realize(conformances: decl.conformances, inScope: parentScope) else {
        return nil
      }

      for trait in traits {
        guard let bases = conformedTraits(of: ^trait, inScope: parentScope) else { return nil }
    ...
    case let t as TraitType:
      ...
      guard
        var work = realize(
          conformances: program.ast[t.decl].refinements, inScope: program.declToScope[t.decl]!)
      else { return nil }

      while let base = work.popFirst() {
        if base == t {
          diagnostics.insert(.diagnose(circularRefinementAt: program.ast[t.decl].identifier.origin))
          return nil
  ...
  mutating func deferTypeChecking(_ expr: NodeID<LambdaExpr>) { pendingLambdas.append(expr) }
  ...
  mutating func checkPending() -> Bool {
    ...
    while let id = pendingLambdas.popLast() {
      if let declType = exprTypes[id]?.base as? LambdaType, !declType.flags.contains(.hasError) {
        ...
        for i in 0..<parameters.count { declTypes[parameters[i]] = declType.inputs[i].type }
  ...
  public mutating func check(module id: NodeID<ModuleDecl>) -> Bool {
    ...
    var success = true
    for decl in program.ast.topLevelDecls(id) { success = check(decl: decl) && success }
  ...
  private mutating func check<T: DeclID>(decl id: T) -> Bool {
    switch id.kind {
    case AssociatedTypeDecl.self: return check(associatedType: NodeID(rawValue: id.rawValue))
    case AssociatedValueDecl.self: return check(associatedValue: NodeID(rawValue: id.rawValue))
    case BindingDecl.self: return check(binding: NodeID(rawValue: id.rawValue))
    case ConformanceDecl.self: return check(conformance: NodeID(rawValue: id.rawValue))
    case FunctionDecl.self: return check(function: NodeID(rawValue: id.rawValue))
    case InitializerDecl.self: return check(initializer: NodeID(rawValue: id.rawValue))
    case MethodDecl.self: return check(method: NodeID(rawValue: id.rawValue))
    case MethodImplDecl.self:
      return check(method: NodeID(rawValue: program.declToScope[id]!.rawValue))
    case OperatorDecl.self: return check(operator: NodeID(rawValue: id.rawValue))
    case ProductTypeDecl.self: return check(productType: NodeID(rawValue: id.rawValue))
    case SubscriptDecl.self: return check(subscript: NodeID(rawValue: id.rawValue))
    case TraitDecl.self: return check(trait: NodeID(rawValue: id.rawValue))
    case TypeAliasDecl.self: return check(typeAlias: NodeID(rawValue: id.rawValue))
    default: unreachable("unexpected declaration")
    }
  }

  private mutating func check(associatedType: NodeID<AssociatedTypeDecl>) -> Bool { return true }

  private mutating func check(associatedValue: NodeID<AssociatedValueDecl>) -> Bool { return true }
  ...
  mutating func check(binding id: NodeID<BindingDecl>) -> Bool {
    ...
    case nil: declRequests[id] = .typeCheckingStarted
    case .typeCheckingStarted:
    ...
    if let initializer = program.ast[id].initializer {
      ...
      let solution = infer(
        expr: initializer, inferredType: initializerType, expectedType: shape.type, inScope: scope,
  ...
  private mutating func check(extension: ExtensionDecl) -> Bool { fatalError("not implemented") }
  ...
  private mutating func _check(function id: NodeID<FunctionDecl>) -> Bool {
    ...
    if program.isNonStaticMember(id) {
      ...
      if let type = functionType.captures.first?.type.base as? RemoteType {
        ...
        declTypes[receiverDecl] = ^ParameterType(
          convention: .sink, bareType: functionType.environment)
      }
    ...
    case .block(let stmt): return check(brace: stmt) && success
  ...
  private mutating func _check(initializer id: NodeID<InitializerDecl>) -> Bool {
    ...
    if program.ast[id].introducer.value == .memberwiseInit { return true }
  ...
  private mutating func check(methodImpl: MethodImplDecl) -> Bool { fatalError("not implemented") }

  private mutating func check(namespace: NamespaceDecl) -> Bool { fatalError("not implemented") }
  ...
  private mutating func check(parameter id: NodeID<ParameterDecl>, siblingNames: inout Set<String>)
    -> Bool
  {
    // Check for duplicate parameter names.
    if !siblingNames.insert(program.ast[id].name).inserted {
      diagnostics.insert(
        .diganose(duplicateParameterNamed: program.ast[id].name, at: program.ast[id].origin))
      declRequests[id] = .failure
      return false
    }
    ...
    if let defaultValue = program.ast[id].defaultValue {
      ...
      let solution = infer(
        expr: defaultValue, inferredType: defaultValueType, expectedType: parameterType.bareType,
        inScope: program.declToScope[id]!, constraints: constraints)
  ...
  private mutating func check(operator id: NodeID<OperatorDecl>) -> Bool {
    ...
    for decl in program.ast[source].decls where decl.kind == OperatorDecl.self {
      ...
      if oper != id, program.ast[oper].notation.value == program.ast[id].notation.value,
        program.ast[oper].name.value == program.ast[id].name.value
      {
        diagnostics.insert(
          .diagnose(duplicateOperatorNamed: program.ast[id].name.value, at: program.ast[id].origin))
        return false
      }
    }
  ...
  private mutating func _check(productType id: NodeID<ProductTypeDecl>) -> Bool {
    ...
    for j in program.ast[id].members { success = check(decl: j) && success }
    ...
    for trait in traits { success = check(conformanceOfProductDecl: id, to: trait) && success }

    // Type check extending declarations.
    let type = declTypes[id]!
    for j in extendingDecls(of: type, exposedTo: container) { success = check(decl: j) && success }
  ...
  private mutating func _check(subscript id: NodeID<SubscriptDecl>) -> Bool {
    ...
    for impl in program.ast[id].impls {
      ...
      if program.isNonStaticMember(id) {
        ...
        declTypes[receiverDecl] = ^ParameterType(
          convention: program.ast[impl].introducer.value.convention, bareType: receiverType.base)
        declRequests[receiverDecl] = .success
      }
  ...
  private mutating func _check(typeAlias id: NodeID<TypeAliasDecl>) -> Bool {
    ...
    switch program.ast[id].body {
    case .typeExpr(let j):
      if let s = realize(j, inScope: AnyScopeID(id))?.instance { subject = s } else { return false }

    case .union: fatalError("not implemented")
    }
  ...
  private mutating func check(`var`: VarDecl) -> Bool { fatalError("not implemented") }
  ...
  private mutating func _check<T: DeclID>(decl id: T, _ action: (inout Self, T) -> Bool) -> Bool {
    ...
    while true {
      ...
      case .typeRealizationCompleted: declRequests[id] = .typeCheckingStarted
  ...
  private mutating func check(
    conformanceOfProductDecl decl: NodeID<ProductTypeDecl>, to trait: TraitType
  ) -> Bool {
    ...
    for j in program.ast[trait.decl].members {
      ...
      case AssociatedValueDecl.self: fatalError("not implemented")
      ...
      case FunctionDecl.self:
        ...
        func substituteSelf(type: AnyType) -> TypeTransformAction {
          ...
          case let t as AssociatedTypeType:
            ...
            let replacement = components.dropLast(1).reversed().reduce(
              into: conformingType,
              { (r, c) in if r.isError { return }
                ...
                case is ConformanceLensType: fatalError("not implemented")
        ...
        if candidates.count > 1 { fatalError("not implemented") }
        ...
        if candidates.isEmpty && (program.ast[requirement].body == nil) {
          diagnostics.insert(
            .diagnose(
              conformingType, doesNotConformTo: trait, at: program.ast[decl].identifier.origin,
              because: [
                .diagnose(
  ...
  private mutating func check<T: StmtID, S: ScopeID>(stmt id: T, inScope lexicalContext: S) -> Bool
  {
    switch id.kind {
    case AssignStmt.self:
      return check(assign: NodeID(rawValue: id.rawValue), inScope: lexicalContext)

    case BraceStmt.self: return check(brace: NodeID(rawValue: id.rawValue))
    ...
    case ExprStmt.self:
      ...
      if let type = infer(expr: stmt.expr, inScope: lexicalContext) {
        ...
        if type != .void {
          diagnostics.insert(.diagnose(unusedResultOfType: type, at: program.ast[stmt.expr].origin))
        }
        return true
      } else {
    ...
    case YieldStmt.self: return check(yield: NodeID(rawValue: id.rawValue), inScope: lexicalContext)
  ...
  mutating func check(brace id: NodeID<BraceStmt>) -> Bool {
    var success = true
    for stmt in program.ast[id].stmts { success = check(stmt: stmt, inScope: id) && success }
    return success
  }

  private mutating func check<S: ScopeID>(assign id: NodeID<AssignStmt>, inScope lexicalContext: S)
    -> Bool
  {
    ...
    let solution = infer(
      expr: AnyExprID(program.ast[id].right), inferredType: rhsType, expectedType: lhsType,
      inScope: AnyScopeID(lexicalContext), constraints: [assignmentConstraint])

    return solution.diagnostics.isEmpty
  }

  private mutating func check<S: ScopeID>(return id: NodeID<ReturnStmt>, inScope lexicalContext: S)
    -> Bool
  {
    ...
    if let returnValue = program.ast[id].value {
      ...
      let solution = infer(
        expr: returnValue, inferredType: inferredReturnType, expectedType: expectedType,
        inScope: AnyScopeID(lexicalContext), constraints: [c])
      return solution.diagnostics.isEmpty
  ...
  private mutating func check<S: ScopeID>(yield id: NodeID<YieldStmt>, inScope lexicalContext: S)
    -> Bool
  {
    // Retreive the expected output type.
    let expectedType = expectedOutputType(in: lexicalContext)!
    ...
    let solution = infer(
      expr: program.ast[id].value, inferredType: inferredReturnType, expectedType: expectedType,
      inScope: AnyScopeID(lexicalContext), constraints: [c])
    return solution.diagnostics.isEmpty
  }
  ...
  private mutating func environment<T: NodeIDProtocol>(of id: T) -> GenericEnvironment? {
    ...
    case TraitDecl.self: return environment(ofTraitDecl: NodeID(rawValue: id.rawValue))
    default: unreachable("unexpected scope")
    }
  }

  /// Returns the generic environment defined by `id`, or `nil` if it is ill-typed.
  private mutating func environment<T: GenericDecl>(ofGenericDecl id: NodeID<T>)
    -> GenericEnvironment?
  {
    switch environments[id] {
    case .done(let e): return e
    case .inProgress: fatalError("circular dependency")
    case nil: environments[id] = .inProgress
    }
    ...
    for p in clause.parameters {
      ...
      guard
        let traits = realize(conformances: list, inScope: program.scopeToParent[AnyScopeID(id)!]!)
      else { return nil }
  ...
  private mutating func environment<T: TypeExtendingDecl>(ofTypeExtendingDecl id: NodeID<T>)
    -> GenericEnvironment?
  {
    switch environments[id] {
    case .done(let e): return e
  ...
  /// Returns the generic environment defined by `i`, or `nil` if it is ill-typed.
  private mutating func environment(ofTraitDecl id: NodeID<TraitDecl>) -> GenericEnvironment? {
    switch environments[id] {
    case .done(let e): return e
    case .inProgress: fatalError("circular dependency")
    case nil: environments[id] = .inProgress
      ...
      case AssociatedTypeDecl.self:
        success =
          associatedConstraints(
            ofType: NodeID(rawValue: member.rawValue), ofTrait: id, into: &constraints) && success

      case AssociatedValueDecl.self:
        success =
          associatedConstraints(
            ofValue: NodeID(rawValue: member.rawValue), ofTrait: id, into: &constraints) && success
  ...
  private mutating func associatedConstraints(
    ofType associatedType: NodeID<AssociatedTypeDecl>, ofTrait trait: NodeID<TraitDecl>,
    ...
    guard let traits = realize(conformances: list, inScope: AnyScopeID(trait)) else { return false }
  ...
  // returns whether they are all well-typed.
  private mutating func associatedConstraints(
    ofValue associatedValue: NodeID<AssociatedValueDecl>, ofTrait trait: NodeID<TraitDecl>,
    into constraints: inout [Constraint]
  ) -> Bool {
    // Realize the LHS of the constraint.
  ...
  private mutating func eval(
    constraintExpr expr: SourceRepresentable<WhereClause.ConstraintExpr>, inScope scope: AnyScopeID
  ) -> Constraint? {
    switch expr.value {
    ...
    case .conformance(let l, let traits):
      ...
      return ConformanceConstraint(
        a, traits: b, because: ConstraintCause(.structural, at: expr.origin))
  ...
  public mutating func infer<S: ScopeID>(
    expr: AnyExprID, expectedType: AnyType? = nil, inScope scope: S
  ) -> AnyType? {
    let solution = infer(
      expr: expr, inferredType: nil, expectedType: expectedType, inScope: AnyScopeID(scope),
      constraints: [])

    if solution.diagnostics.isEmpty { return exprTypes[expr]! } else { return nil }
  }

  /// Infers the type of `expr` and returns the best solution found by the constraint solver.
  mutating func infer(
    expr: AnyExprID, inferredType: AnyType?, expectedType: AnyType?, inScope scope: AnyScopeID,
    constraints: [Constraint]
  ) -> Solution {
    // Generate constraints.
    var generator = ConstraintGenerator(
      scope: scope, expr: expr, inferredType: inferredType, expectedType: expectedType)
    let constraintGeneration = generator.apply(using: &self)

    // Solve the constraints.
    var solver = ConstraintSolver(
      scope: scope, fresh: constraints + constraintGeneration.constraints,
    ...
    for (name, ref) in solution.bindingAssumptions { referredDecls[name] = ref }
  ...
  private mutating func infer<T: PatternID>(pattern: T, inScope scope: AnyScopeID) -> (
    type: AnyType, constraints: [Constraint], decls: [NodeID<VarDecl>]
  )? {
    var constraints: [Constraint] = []
    var decls: [NodeID<VarDecl>] = []
    if let type = _infer(
      pattern: pattern, expectedType: nil, inScope: scope, constraints: &constraints, decls: &decls)
    {
  ...
  private mutating func _infer<T: PatternID>(
    pattern: T, expectedType: AnyType?, inScope scope: AnyScopeID, constraints: inout [Constraint],
    ...
    case BindingPattern.self:
      ...
      return _infer(
        pattern: lhs.subpattern, expectedType: subpatternType, inScope: scope,
        constraints: &constraints, decls: &decls)

    case _ where pattern.kind.value is Expr.Type: fatalError("not implemented")
    ...
    case TuplePattern.self:
      ...
      case let rhs as TupleType:
        ...
        if rhs.elements.count == lhs.elements.count {
          ...
          for (a, b) in zip(lhs.elements, rhs.elements) {
            if _infer(
              pattern: a.pattern, expectedType: b.type, inScope: scope, constraints: &constraints,
              decls: &decls) == nil
            {
              return nil
            }
          ...
          if lLabels == rLabels {
            ...
            diagnostics.insert(
              .diagnose(labels: lLabels, incompatibleWith: rLabels, at: program.ast[pattern].origin)
            )
            return nil
          }
        } else {
          ...
          diagnostics.insert(
            .diagnose(invalidDestructuringOfType: expectedType!, at: program.ast[pattern].origin))
          return nil
        }
      ...
      case nil:
        ...
        for element in lhs.elements {
          guard
            let type = _infer(
              pattern: element.pattern, expectedType: nil, inScope: scope,
              constraints: &constraints, decls: &decls)
          else { return nil }
    ...
    case WildcardPattern.self: return expectedType ?? ^TypeVariable()
  ...
  mutating func resolve(
    _ name: Name, introducedInDeclSpaceOf lookupContext: AnyScopeID? = nil,
    ...
    return matches.compactMap({ (match) -> (AnyDeclID, AnyType)? in
      ...
      if let t = matchType.base as? ParameterType { matchType = t.bareType }
  ...
  mutating func lookup(unqualified name: String, inScope scope: AnyScopeID) -> DeclSet {
    ...
    for scope in program.scopes(from: scope) {
      ...
      if newMatches.contains(where: { (i) in !(program.ast[i] is FunctionDecl) }) { return matches }
    }
    ...
    if program.ast[root]?.name == name { return [AnyDeclID(root!)] }
  ...
  mutating func lookup<T: ScopeID>(
    _ name: String, introducedInDeclSpaceOf lookupContext: T, inScope origin: AnyScopeID
  ) -> DeclSet {
    switch lookupContext.kind {
    case ProductTypeDecl.self:
      let type = ^ProductType(NodeID(rawValue: lookupContext.rawValue), ast: program.ast)
    ...
    default: return names(introducedIn: lookupContext)[name, default: []]
    }
  }

  /// Returns the declarations that introduce `name` as a member of `type` in `scope`.
  mutating func lookup(_ name: String, memberOf type: AnyType, inScope scope: AnyScopeID) -> DeclSet
  {
    ...
    if let m = memberLookupTables[key]?[name] { return m }
    ...
    case let t as ProductType:
      ...
      if name == "init" { matches.insert(AnyDeclID(program.ast[t.decl].memberwiseInit)) }

    case let t as TraitType: matches = names(introducedIn: t.decl)[name, default: []]

    case let t as TypeAliasType: matches = names(introducedIn: t.decl)[name, default: []]

    default: matches = DeclSet()
    }

    // We're done if we found at least one non-overloadable match.
    if matches.contains(where: { i in !(program.ast[i] is FunctionDecl) }) { return matches }
  ...
  func lookup(
    operator operatorName: Identifier, notation: OperatorNotation, inScope scope: AnyScopeID
  ) -> [NodeID<OperatorDecl>] {
  ...
  func lookup(
    operator operatorName: Identifier, notation: OperatorNotation, in module: NodeID<ModuleDecl>
  ) -> NodeID<OperatorDecl>? {
  ...
  private mutating func extendingDecls<S: ScopeID>(of subject: AnyType, exposedTo scope: S)
    -> [AnyDeclID]
  {
    ...
    for scope in program.scopes(from: scope) {
      ...
      case ModuleDecl.self:
        ...
        insert(
          into: &matches, decls: program.ast.topLevelDecls(module), extending: canonicalSubject,
          inScope: scope)
        root = module

      case TopLevelDeclSet.self: continue
  ...
  private mutating func insert<S: Sequence>(
    into matches: inout [AnyDeclID], decls: S, extending subject: AnyType, inScope scope: AnyScopeID
    ...
    for i in decls where i.kind == ConformanceDecl.self || i.kind == ExtensionDecl.self {
      ...
      if canonicalize(type: extendedType.instance) == subject { matches.append(i) }
    }
  }
  ...
  private func names<T: NodeIDProtocol>(introducedIn scope: T) -> LookupTable {
    if let module = NodeID<ModuleDecl>(scope) {
      return program.ast[module].sources.reduce(
        into: [:],
        { (table, s) in table.merge(names(introducedIn: s), uniquingKeysWith: { (l, _) in l }) })
    }
    ...
    for id in decls {
      switch id.kind {
      case AssociatedValueDecl.self, AssociatedTypeDecl.self, GenericParameterDecl.self,
        NamespaceDecl.self, ParameterDecl.self, ProductTypeDecl.self, TraitDecl.self,
        TypeAliasDecl.self, VarDecl.self:
        let name = (program.ast[id] as! SingleEntityDecl).name
        table[name, default: []].insert(id)

      case BindingDecl.self, ConformanceDecl.self, MethodImplDecl.self, OperatorDecl.self,
      ...
      case InitializerDecl.self: table["init", default: []].insert(id)
  ...
  mutating func realize(_ expr: AnyExprID, inScope scope: AnyScopeID) -> MetatypeType? {
    ...
    case NameExpr.self: return realize(name: NodeID(rawValue: expr.rawValue), inScope: scope)
    ...
    case TupleTypeExpr.self: return realize(tuple: NodeID(rawValue: expr.rawValue), inScope: scope)

    case WildcardExpr.self: return MetatypeType(of: TypeVariable(node: expr.base))
  ...
  private mutating func realizeMagicTypeExpr(_ expr: NodeID<NameExpr>, inScope scope: AnyScopeID)
    -> MetatypeType?
  {
  ...
  private mutating func realize(
    conformanceLens id: NodeID<ConformanceLensTypeExpr>, inScope scope: AnyScopeID
  ) -> MetatypeType? {
    ...
    guard let traits = conformedTraits(of: subject, inScope: scope), traits.contains(lensTrait)
    else {
      diagnostics.insert(
  ...
  private mutating func realize(lambda id: NodeID<LambdaTypeExpr>, inScope scope: AnyScopeID)
    -> MetatypeType?
  {
    let node = program.ast[id]
    ...
    return MetatypeType(
      of: LambdaType(
        receiverEffect: node.receiverEffect?.value, environment: environment, inputs: inputs,
        output: output))
  }

  private mutating func realize(name id: NodeID<NameExpr>, inScope scope: AnyScopeID)
    -> MetatypeType?
  {
    ...
    case .implicit:
      diagnostics.insert(.diagnose(notEnoughContextToResolveMember: name.value, at: name.origin))
      return nil
    }
    ...
    if match.kind == AssociatedTypeDecl.self {
      ...
      case is AssociatedTypeType, is ConformanceLensType, is GenericTypeParameterType:
        referredType = MetatypeType(of: AssociatedTypeType(decl, domain: domain!, ast: program.ast))
      ...
      case nil:
        ...
        let instance = AssociatedTypeType(
          NodeID(rawValue: match.rawValue), domain: domain, ast: program.ast)
        referredType = MetatypeType(of: instance)

      case .some:
        diagnostics.insert(
          .diagnose(invalidUseOfAssociatedType: program.ast[decl].name, at: name.origin))
        return nil
      }
    } else {
  ...
  private mutating func realize(parameter id: NodeID<ParameterTypeExpr>, inScope scope: AnyScopeID)
    -> MetatypeType?
  {
    let node = program.ast[id]
  ...
  private mutating func realize(tuple id: NodeID<TupleTypeExpr>, inScope scope: AnyScopeID)
    -> MetatypeType?
  {
    var elements: [TupleType.Element] = []
  ...
  private mutating func realize(conformances: [NodeID<NameExpr>], inScope scope: AnyScopeID) -> Set<
    TraitType
  >? {
  ...
  mutating func realize<T: DeclID>(decl id: T) -> AnyType {
    ...
    case BindingDecl.self: return realize(bindingDecl: NodeID(rawValue: id.rawValue))

    case ConformanceDecl.self, ExtensionDecl.self:
      return _realize(
        decl: id,
        { (this, id) in let decl = this.program.ast[id] as! TypeExtendingDecl
          let type = this.realize(decl.subject, inScope: this.program.declToScope[id]!)
          return type.flatMap(AnyType.init(_:))
        })

    case FunctionDecl.self: return realize(functionDecl: NodeID(rawValue: id.rawValue))

    case InitializerDecl.self: return realize(initializerDecl: NodeID(rawValue: id.rawValue))

    case MethodDecl.self: return realize(methodDecl: NodeID(rawValue: id.rawValue))

    case MethodImplDecl.self:
      return realize(methodDecl: NodeID(rawValue: program.declToScope[id]!.rawValue))

    case ParameterDecl.self: return realize(parameterDecl: NodeID(rawValue: id.rawValue))
    ...
    case SubscriptDecl.self: return realize(subscriptDecl: NodeID(rawValue: id.rawValue))
    ...
    case VarDecl.self:
      ...
      return bindingType.isError ? bindingType : declTypes[id]!
  ...
  private mutating func _realize(functionDecl id: NodeID<FunctionDecl>) -> AnyType {
    ...
    for i in program.ast[id].parameters {
      ...
      if let annotation = program.ast[i].annotation {
        ...
        if program.ast[id].isInExprContext {
          ...
          declRequests[i] = .typeRealizationCompleted
          inputs.append(
            CallableTypeParameter(label: program.ast[i].label?.value, type: parameterType))
        } else {
          unreachable("expected type annotation")
        }
      }
    ...
    guard
      let explicitCaptureTypes = realize(
        explicitCaptures: program.ast[id].explicitCaptures, collectingNamesIn: &explicitCaptureNames
      )
    else { return .error }

    let implicitCaptures: [ImplicitCapture] =
      program.isLocal(id) ? realize(implicitCapturesIn: id, ignoring: explicitCaptureNames) : []
    ...
    var receiver: AnyType? =
      isNonStaticMember ? realizeSelfTypeExpr(inScope: program.declToScope[id]!)!.instance : nil
    ...
    if isNonStaticMember {
      ...
      return ^LambdaType(
        receiverEffect: effect, environment: receiver!, inputs: inputs, output: outputType)
    } else {
      // Create a regular lambda.
      let environment = TupleType(
        explicitCaptureTypes.map({ (t) -> TupleType.Element in .init(label: nil, type: t) })
          + implicitCaptures.map({ (c) -> TupleType.Element in .init(label: nil, type: ^c.type) }))
  ...
  public mutating func realize(genericParameterDecl id: NodeID<GenericParameterDecl>) -> AnyType {
    _realize(decl: id, { (this, id) in this._realize(genericParameterDecl: id) })
  }

  private mutating func _realize(genericParameterDecl id: NodeID<GenericParameterDecl>) -> AnyType {
  ...
  private mutating func _realize(initializerDecl id: NodeID<InitializerDecl>) -> AnyType {
    ...
    let receiverParameterType = CallableTypeParameter(
      label: "self", type: ^ParameterType(convention: .set, bareType: receiverType))
  ...
  private mutating func _realize(methodDecl id: NodeID<MethodDecl>) -> AnyType {
    ...
    if capabilities.contains(.inout) && (outputType != receiver) {
      let range =
        program.ast[id].output.map({ (output) in program.ast[output].origin })
    ...
    return ^MethodType(
      capabilities: capabilities, receiver: receiver, inputs: inputs, output: outputType)
  }
  ...
  private mutating func realize(parameterDecl id: NodeID<ParameterDecl>) -> AnyType {
    ...
    case .typeRealizationCompleted, .typeCheckingStarted, .success, .failure: return declTypes[id]!
    }
  }
  ...
  private mutating func _realize(subscriptDecl id: NodeID<SubscriptDecl>) -> AnyType {
    ...
    return ^SubscriptType(
      isProperty: program.ast[id].parameters == nil, capabilities: capabilities,
      environment: ^environment, inputs: inputs, output: output)
  }
  ...
  private mutating func realize(
    explicitCaptures list: [NodeID<BindingDecl>], collectingNamesIn explictNames: inout Set<Name>
  ) -> [AnyType]? {
    ...
    for i in list {
      ...
      for (_, namePattern) in program.ast.names(in: program.ast[i].pattern) {
        ...
        if !explictNames.insert(Name(stem: program.ast[varDecl].name)).inserted {
          diagnostics.insert(
            .diagnose(
              duplicateCaptureNamed: program.ast[varDecl].name, at: program.ast[varDecl].origin))
          success = false
        }
      }
      ...
      if type.isError {
        ...
        case .let: captures.append(^RemoteType(.let, type))
        case .inout: captures.append(^RemoteType(.inout, type))
        case .sinklet, .var: captures.append(type)
        }
      }
    }
  ...
  private mutating func realize<T: Decl & LexicalScope>(
    implicitCapturesIn decl: NodeID<T>, ignoring explictNames: Set<Name>
  ) -> [ImplicitCapture] {
    ...
    for (name, uses) in collector.freeNames(in: decl) {
      ...
      if program.isMember(captureDecl) {
        ...
        let innermostTypeScope = program.scopes(from: program.scopeToParent[decl]!).first(where: {
          $0.kind.value is TypeScope.Type
        })!
        ...
        if let i = receiverIndex, uses.capability != .let {
          ...
          captures[i] = ImplicitCapture(
            name: captures[i].name, type: RemoteType(.inout, captures[i].type.base),
          ...
          case 0: continue
          case 1: receiverDecl = matches.first!
          default: unreachable()
          }
          ...
          captures.append(
            ImplicitCapture(
              name: Name(stem: "self"), type: RemoteType(uses.capability, receiverType.skolemized),
              decl: receiverDecl))
        }
      ...
      captures.append(
        ImplicitCapture(
          name: name, type: RemoteType(uses.capability, captureType), decl: captureDecl))
    }
  ...
  private mutating func _realize<T: DeclID>(decl id: T, _ action: (inout Self, T) -> AnyType?)
    -> AnyType
  {
    // Check if a type realization request has already been received.
    switch declRequests[id] {
    case nil: declRequests[id] = .typeRealizationStarted
  ...
  private mutating func memberwiseInitType(of decl: NodeID<ProductTypeDecl>) -> LambdaType? {
    ...
    inputs.append(
      CallableTypeParameter(
        label: "self", type: ^ParameterType(convention: .set, bareType: receiver)))
  ...
  func open(type: AnyType) -> (AnyType, ConstraintSet) {
    ...
    func _impl(type: AnyType) -> TypeTransformAction {
      switch type.base {
      case is AssociatedTypeType: fatalError("not implemented")
      ...
      case is GenericValueParameterType: fatalError("not implemented")
  ...
  func contextualize<S: ScopeID>(type: AnyType, inScope scope: S, cause: ConstraintCause) -> (
  ...
  mutating func doesSupportInitSugar(_ decl: AnyDeclID) -> Bool {
    switch decl.kind {
    case AssociatedTypeDecl.self, ProductTypeDecl.self, TypeAliasDecl.self: return true
