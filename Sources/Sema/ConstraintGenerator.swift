...
fileprivate struct ConstraintVisitor: NodeVisitor {
  ...
  /// Realizes the semantic type denoted by the given representation.
  ///
  /// A type representation can either denote three kinds of types:
  ///
  ...
  func realize(typeOf typeRepr: TypeRepr) -> ValType {
    // Third case, the representation refers to an existential type.
    if let type = typeRepr.type as? GenericParamType {
      // FIXME: A lot of magic will have to happen here to handle associated and dependent types.
      if let existential = gen.existentials[HashableBox(type)] {
        return existential
      }

      let existential = gen.context.existentialType()
      gen.existentials[HashableBox(type)] = existential
      return existential
    }

    // Second case, the type is an unspecialized generic.
    if typeRepr.type.props.contains(.hasTypeParams) {
      return typeRepr.type.opened
    }

    // First case, the type is concrete.
  ...
  func visit(_ node: PatternBindingDecl) {
    ...
    if let sign = node.typeSign {
      let signType = realize(typeOf: sign)
      gen.system.insert(
        EqualityConstraint(node.pattern.type, isEqualTo: signType,
  ...
  func visit(_ node: FunParamDecl) {
    ...
    if let sign = node.typeSign {
      let signType = realize(typeOf: sign)
      gen.system.insert(
        EqualityConstraint(node.type, isEqualTo: signType,
  ...
  func visit(_ node: RetStmt) {
    ...
    let retType = funDecl.retTypeSign.map(realize(typeOf:)) ?? gen.context.unitType
    let valType = node.value?.type ?? gen.context.unitType
    gen.system.insert(
      SubtypingConstraint(valType, isSubtypeOf: retType,
