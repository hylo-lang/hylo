...
public struct R1CS {
  ...
  public static let unitWire = WireID(rawValue: 0)
...
extension R1CS: CustomDebugStringConvertible {
  public var debugDescription: String {
    ...
    output += "\(bold)\(cyan)║\(reset)  \(bold)R1CS Constraint System\(reset)                                       \(bold)\(cyan)║\(reset)\n"
    output += "\(bold)\(cyan)╚═══════════════════════════════════════════════════════════════╝\(reset)\n\n"
...
extension LinearCombination: CustomDebugStringConvertible {
  ...
  func debugDescription(compact: Bool) -> String {
    ...
    if compact {
      ...
      var parts: [String] = []
      for term in terms {
        let wireStr = term.wire.rawValue == 0 ? "\(yellow)1\(reset)" : "\(green)w\(term.wire.rawValue)\(reset)"
      ...
      var parts: [String] = []
      for term in terms {
        let wireStr = term.wire.rawValue == 0 ? "\(yellow)w\(term.wire.rawValue) (1)\(reset)" : "\(green)w\(term.wire.rawValue)\(reset)"
        parts.append("\(magenta)\(term.coefficient)\(reset) \(dim)·\(reset) \(wireStr)")
      }
      return parts.joined(separator: " \(dim)+\(reset) ")
    }
