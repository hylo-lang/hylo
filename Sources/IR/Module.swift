...
public struct Module {
  ...
  /// Modifies the IR of `function` with `action`.
  @discardableResult
  mutating func modifyIR<T>(of function: Function.ID, with action: (inout IRWriter) -> T) -> T {
    IRWriter.withInstance(for: function, with: &functions[function]!, action)
  }

}

extension Module: IRInspector {
