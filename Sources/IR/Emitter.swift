...
struct Emitter {
  ...
  private mutating func lowerInClearContext(function d: FunctionDecl.ID) -> Function.ID {
    ...
    case .block(let s):
      ...
      let returnSite = inFrame(bodyFrame) { $0.returnSite(lowering: s, output: returnType) }
      _lowering(at: returnSite) { $0._return() }

    case .expr(let e):
      _lowering(e) { me in
        me.pushing(bodyFrame, { $0._emitStore(e, in: $0.returnValue!) })
        me._return()
      }
    }
  ...
  /// site of the return statement.
  @discardableResult
  private mutating func returnSite(
    lowering b: BraceStmt.ID, output returnType: AnyType
  ) -> SourceRange {
    switch emit(braceStmt: b) {
    case .next:
      if canonical(returnType).isVoidOrNever {
        _lowering(after: b) { e in
          e._mark_state(.initialized, e.returnValue!)
        }
      }
      return ast[b].site
  ...
  private mutating func lower(ffi d: FunctionDecl.ID) {
    ...
    _lowering(d) { e in
      ...
      let arguments = e.module[entry].inputs.indices.dropLast().map {
        e._convert_to_foreign(.parameter(entry, $0))
      }

      // Return type must be foreign convertible unless it is `Void` or `Never`.
      let t = e.module.functions[f]!.output
      let returnType = t.isVoidOrNever ? t
        : e.program.foreignRepresentation(of: t, exposedTo: e.insertionScope!)

      let foreignResult = e._call_ffi(e.program[d].attributes.foreignName!, on: arguments, returning: returnType)
      ...
      default:
        let v = e._emitConvert(foreign: foreignResult, to: e.module.functions[f]!.output)
        e._move(.set, v, to: e.returnValue!)
      }
      e._dealloc_top_frame()
      e._return()
  ...
  private mutating func lower(initializer d: InitializerDecl.ID) {
    ...
    let returnSite = _frame(locals: locals) {
      $0.returnSite(lowering: $0.ast[d].body!, output: .void)
    }
    ...
    _lowering(d) { e in
      if l.properties.isEmpty {
        e._mark_state(.initialized, entry.parameter(0))
      }
    }
    _lowering(at: returnSite) { $0._return() }
  }
  ...
  private mutating func lower(methodImpl d: MethodImpl.ID) {
    ...
    case .block(let s):
      let returnSite = _frame(locals: parameters) {
        $0.returnSite(lowering: s, output: ArrowType($0.program[d].type)!.output)
      }
      _lowering(at: returnSite) { $0._return() }

    case .expr(let e):
      _lowering(e) { me in
        me._frame(locals: parameters) { me in
          me._emitStore(e, in: me.returnValue!)
        }
        me._return()
      }
    }
  ...
  private mutating func lower(subscriptImpl d: SubscriptImpl.ID) {
    ...
    case .expr(let e):
      _lowering(e) { this in
        this._frame(locals: locals) { (this) in
  ...
  private mutating func emitInitStoredLocalBindings(
    ...
    _lowering(lhs) { e in
      let rhs = e._subfield_view(storage, at: subfield)
      e.emitStore(initializer, in: rhs)
      e.emitLocalDeclarations(introducedBy: lhs, referringTo: subfield, relativeTo: storage)
    }
  }
  ...
  private mutating func emitLocalDeclaration(
    ...
    _lowering(name) { e in
      let s = e._subfield_view(storage, at: subfield)
      e.frames[e.ast[name].decl] = s
      return s
    }
  }
  ...
  private mutating func assignProjections(of rhs: Operand, to d: BindingPattern.ID) {
    ...
    for (path, name) in ast.names(in: program[d].subpattern) {
      ...
      _lowering(partDecl) { e in
        part = e._emitCoerce(part, to: bindingType)
        e.frames[partDecl] = e._access(request, from: part, correspondingTo: partDecl)
      }
  ...
  private mutating func withPrologue(
    ...
    withClearContext { (me: inout Emitter) -> Function.ID in
  ...
  private mutating func lower(syntheticAutoclosure d: SynthesizedFunctionDecl) -> Function.ID {
    ...
    // Emit the body.
    _lowering(argument) { e in
      e._emitStore(argument, in: e.returnValue!)
      e._return()
    }
  ...
  private mutating func emitControlFlow(
    ...
    case .next:
      handleNext(&self)
    case .return(let s):
      _lowering(s) { $0._emitControlFlow(return: s) }
    case .break(let s):
      _lowering(s) { $0._emitControlFlow(break: s) }
    default:
      UNIMPLEMENTED()
    }
  }
  ...
  private mutating func _emitControlFlow(return s: ReturnStmt.ID) {
  ...
  private mutating func _emitControlFlow(break s: BreakStmt.ID) {
  ...
  private mutating func emit(braceStmt s: BraceStmt.ID) -> ControlFlow {
    frames.push()
    defer {
      _lowering(after: s) { $0._dealloc_top_frame() }
      frames.pop()
    }
  ...
  private mutating func emit(consumingForStmt s: ForStmt.ID) -> ControlFlow {
    ...
    let element =  _lowering(at: introducer) { e in
      e._alloc_stack(^e.ast.optional(witness.element))
    }
    ...
    _lowering(at: introducer) { e in
      e._branch(to: head)
      e.insertionPoint = .end(of: head)

      let x0 = e._access(.inout, from: domain)
      e._emitApply(witness.next, to: [x0], writingResultTo: element)
      e._end_access(x0)
    }
    ...
    _lowering(at: introducer) { e in
      let flow = e.emit(braceStmt: e.ast[s].body)
      e.emitControlFlow(flow) { (me) in
  ...
  private mutating func emit(nonConsumingForStmt s: ForStmt.ID) -> ControlFlow {
    ...
    _lowering(at: introducer) { e in
      let quit = e._alloc_stack(^e.ast.coreType("Bool")!)
      // The start and end positions of the collection; the former is updated with each iteration.
      let (currentPosition, endPosition) = e._emitPositions(
        forIteratingOver: domain, usingWitness: collectionWitness)

      // The "head" of the loop; tests for the exit condition.
      let head = e.appendBlock(in: s)
      // The "lens" of the loop; tests for narrowing conditions and applies filters.
      let enter = e.appendBlock(in: s)
      // The "tail" of the loop; increments the index and jumps back to the head.
      let tail = e.appendBlock(in: s)
      // The remainder of the program, after the loop.
      let exit = e.appendBlock()

      e.loops.append(LoopID(depth: e.frames.depth, exit: exit))
      defer { e.loops.removeLast() }

      e._branch(to: head)

      e.insertionPoint = .end(of: head)
      let x0 = e._access(.let, from: currentPosition)
      let x1 = e._access(.let, from: endPosition)
      e._emitApply(.constant(equal), to: [x0, x1], writingResultTo: quit)
      e._end_access(x1)
      e._end_access(x0)
      let x2 = e._emitLoadBuiltinBool(quit)
      e._cond_branch(if: x2, then: exit, else: enter)

      e.insertionPoint = .end(of: enter)
      let x6 = e._access(.let, from: domain)
      let x7 = e._access(.let, from: currentPosition)
      ...
      if e.module.type(of: x8).ast != collectionWitness.element {
        UNIMPLEMENTED("narrowing projections #1099")
      }

      e.emitLocalDeclarations(
        introducedBy: e.program[s].binding.pattern, referringTo: [], relativeTo: x8)

      // TODO: Filter
      precondition(e.ast[s].filter == nil, "loop filters are not implemented")
      ...
      e.insertionPoint = .end(of: tail)
      let x3 = e._alloc_stack(collectionWitness.position)
      let x4 = e._access(.let, from: domain)
      let x5 = e._access(.let, from: currentPosition)
      e._emitApply(collectionWitness.positionAfter, to: [x4, x5], writingResultTo: x3)
      e._end_access(x4)
      e._end_access(x5)
      e._move (.inout, x3, to: currentPosition)
      e._dealloc_stack(x3)
      e._branch(to: head)

      e.insertionPoint = .end(of: exit)
    }
    return .next
  }
  ...
  private mutating func emitStore<T: ExprID>(value e: T) -> Operand {
    let s = _lowering(e) { this in
      let r = this._alloc_stack(this.program[e].type)
      this.emitStore(e, in: s)
      return r
    }
    return s
  }
  ...
  private mutating func emitStore<T: ExprID>(_ e: T, in storage: Operand) {
    _lowering(e) { $0._emitStore(e, in: storage) }
  }

  /// Inserts the IR for storing the value of `e` to `storage`.
  ///
  /// Precondition: `_site == ast[e].site`
  private mutating func _emitStore<T: ExprID>(_ e: T, in storage: Operand) {
    precondition(_site == ast[e].site)
    switch e.kind {
    case BooleanLiteralExpr.self:
      _emitStore(boolean: ast[BooleanLiteralExpr.ID(e)!].value, to: storage)
    case BufferLiteralExpr.self:
      _emitStore(BufferLiteralExpr.ID(e)!, to: storage)
  ...
  private mutating func emitStore(upcast e: CastExpr.ID, to storage: Operand) {
    ...
    if let u = UnionType(target), u.elements.contains(source) {
      _lowering(e) { me in
        ...
        me.emitStore(me.ast[e].left, in: x0)
        me._close_union(x0)
      }
      return
    }
  ...
  private mutating func emitStore(pointerConversion e: CastExpr.ID, to storage: Operand) {
    ...
    _lowering(e) {
      $0._move([.inout, .set], x0, to: storage)
    }
  }
  ...
  private mutating func emitStore(_ e: ConditionalExpr.ID, to storage: Operand) {
    ...
    _lowering(e) { me in
      ...
      me._frame { $0.emitStore($0.ast[e].failure.value, in: storage) }
      me._branch(to: tail)
    }
    insertionPoint = .end(of: tail)
  }
  ...
  private mutating func emit(
    ...
    _lowering(at: ast.site(of: e)) { _ in }
    switch e {
  ...
  private mutating func pushing<T>(_ newFrame: Frame, _ action: (inout Self) -> T) -> T {
    ...
    defer {
      ...
      _lowering(after: insertionScope!) { _ in }
      _dealloc_top_frame()
      frames.pop()
      _site = savedSource
    }
