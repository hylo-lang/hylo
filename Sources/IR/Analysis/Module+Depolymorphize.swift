    ...
    func rewrite(return i: InstructionID, to b: Block.ID) {
      ...
      let j = modify(&modules[target]!) { (m) in
        for i in rewrittenGenericValue.values.reversed() {
          m[result].insert(m[result].makeDeallocStack(for: .register(i), at: s.site), at: .end(of: b))
        }
        return m[result].insert(m[result].makeReturn(at: s.site), at: .end(of: b))
      }
      monomorphizer.rewrittenInstruction[i] = j
    }
  }
...
extension Module {
  ...
  fileprivate mutating func defineGenericValueArguments(
    ...
    for (k, v) in specialization {
      ...
      let i = self[monomorphized].makeAllocStack(^program.ast.coreType("Int")!, at: insertionSite);
      let s = self[monomorphized].insert(i, at: .end(of: entry))
