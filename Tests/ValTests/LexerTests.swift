...
final class LexerTests: XCTestCase {
  ...
  func testBool() {
    let input = SourceFile(text: "true false")
    assert(
      tokenize(input),
      matches: [
  ...
  func testDecimalInteger() {
    let input = SourceFile(text: "0 001 42 00 1_234 1_2__34__")
    assert(
      tokenize(input),
      matches: [
  ...
  func testHexadecimalInteger() {
    let input = SourceFile(text: "0x0123 0xabcdef 0x__0_a_ 0xg 0x")
    assert(
      tokenize(input),
      matches: [
  ...
  func testOctalInteger() {
    let input = SourceFile(text: "0o0123 0o__0_6_ 0o8 0o")
    assert(
      tokenize(input),
      matches: [
  ...
  func testBinaryInteger() {
    let input = SourceFile(text: "0b01 0b__0_1_ 0b8 0b")
    assert(
      tokenize(input),
      matches: [
  ...
  func testFloatingPoint() {
    let input = SourceFile(text: "0.0 001.00 0.1_2__34__ 1e1_000 1.12e+123 3.45E-6 1. 1e")
    assert(
      tokenize(input),
      matches: [
  ...
  func testString() {
    let input = SourceFile(text: #""" "a 0+ " "a\nb" "a\"" "abc "#)
    assert(
      tokenize(input),
      matches: [
  ...
  func testIdentifiers() {
    let input = SourceFile(text: "foo éléphant _bar _1_2_3 _")
    assert(
      tokenize(input),
      matches: [
  ...
  func testBackquotedIdentifier() {
    let input = SourceFile(text: "`type` `foo` `a_b_` `12`")
    assert(
      tokenize(input),
      matches: [
  ...
  func testAttributes() {
    let input = SourceFile(text: "@implicitcopy @_foo @2")
    assert(
      tokenize(input),
      matches: [
  ...
  func testCastOperators() {
    let input = SourceFile(text: "is as as!")
    assert(
      tokenize(input),
      matches: [
