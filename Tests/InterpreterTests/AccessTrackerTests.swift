...
final class AccessTrackerTests: XCTestCase {
  ...
  func testEndAccess() throws {
    for p in [.inout, .sink, .set] as [AccessKind] {
      ...
      let a4 = try t.begin(p, at: ["b"])  // can start new access after no overlapping subpart
      try t.end(a4, at: ["b"])
      try t.end(a1, at: ["b"])
    ...
    try t.end(a2, at: ["b", "c"])
    try t.end(a3, at: ["b", "c"])
    let a4 = try t.begin(.inout, at: ["b"])  // can start new access after no overlapping subpart
    try t.end(a4, at: ["b"])
